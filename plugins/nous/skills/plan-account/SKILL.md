...
## Tools
- `mcp__nous__get_account` — the full record: facts, Intel, buying committee, timeline, ICP.
- `mcp__nous__get_context` — pass `intent: "account_review"` (or `meeting_prep`) for a task-shaped read when the account is large.
- `mcp__nous__score` — the live ICP fit + intent if the record doesn't already carry it.

## Workflow
1. Resolve the account: call `mcp__nous__get_account` with the email, domain, or entity id the user gave. If a name is ambiguous, ask which one from the candidates it returns.
